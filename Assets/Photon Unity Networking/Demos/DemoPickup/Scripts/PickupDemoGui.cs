...
public class PickupDemoGui : MonoBehaviour
{
    ...
    public void OnGUI()
    {
        ...
        if (this.ShowStartButton && PhotonNetwork.isMasterClient)
        {
            if (GUILayout.Button("Start Game"))
            {
                ...
                PhotonNetwork.LoadLevel("BumperBotScene");
                

            }
        }
        if (PhotonNetwork.player.GetScore() == 8)
        {
            
            GUILayout.MaxWidth(300);
            GUIStyle test = new GUIStyle();
            test.fontSize = 150;
            GUILayout.Label("Your Win!!!!!", test);
